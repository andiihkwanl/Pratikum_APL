...
void tampilkanSemuaSepatu(const vector<Sepatu> &daftarSepatu) {
    if (daftarSepatu.empty()) {
        ...
        for (size_t i = 0; i < daftarSepatu.size(); ++i) {
            cout << "Indeks: " << i << ", Merk: " << daftarSepatu[i].merk << ", Nama Sepatu: " << daftarSepatu[i].namasepatu << ", Ukuran: " << daftarSepatu[i].ukuran << ", Harga: " << daftarSepatu[i].harga << ", Warna: " << daftarSepatu[i].detail.warna << ", Material: " << daftarSepatu[i].detail.material << ", Fitur: ";
            for (int j = 0; j < 2; ++j) {
                cout << daftarSepatu[i].detail.tambahan_terkait_sepatu[j] << " ";
            }
            cout << endl;
        }
    }
